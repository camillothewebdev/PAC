...
function PageSection() {
  ...
  let locationIdNiveau = parseInt(useLocation().pathname.split("/")[2]);
  const [sectionFormData, setSectionFormData] = useState({
    id: "",
    libelle: "",
    adresse: "",
    type: "section",
    idFederation: locationIdNiveau,
  });
  ...
  const fetchNiveaux = async () => {
    ...
    try {
      let res = await axios.get(
        connection + "/lire-section/" + locationIdNiveau
      );
      setNiveaux(res.data);
      setLoading(false);
    } catch (error) {
  ...
  const add = async (event) => {
    if (sectionFormData.libelle == "" || sectionFormData.adresse == "") {
    ...
    niveaux.forEach((niveau) => {
      if (niveau.idNiveau) {
        ...
        if (niveau.nombreNiveaux == 1) {
          ...
          nombre = "Aucune cellules";
        }
        listNiveaux.push(
          <div className="niveau" key={niveau.idNiveau}>
            <div
              onClick={() => {
                navigate(`/cellule/${locationIdNiveau}-${niveau.idNiveau}`);
              }}
              style={{
                padding: "0.5em",
                borderRadius: "13px",
            ...
            >
              ...
              <p style={{ textTransform: "capitalize" }}>{niveau.adresse}</p>
